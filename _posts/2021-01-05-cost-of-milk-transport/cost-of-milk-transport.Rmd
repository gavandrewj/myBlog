...
```{r}
#options(scipen=999)
males <- as.matrix(subset(data,truck == "gasoline")[,1:3])
females <- as.matrix(subset(data ,truck == "dies")[,1:3])
...
length <- pairwiseCI(data = data , length ~ truck, conf.level = 0.95/6)
width <- pairwiseCI(data = data , width ~ truck, conf.level = 0.95/6)
height <- pairwiseCI(data = data , height ~ truck, conf.level = 0.95/6)
