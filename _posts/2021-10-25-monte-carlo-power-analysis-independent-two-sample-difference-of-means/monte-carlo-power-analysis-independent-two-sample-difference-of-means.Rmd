...
dataframes <- lapply(1:50,build_sample,2,4.5,1,1,50,1,1)
...
grouptwo_info <- allocation_data %>% 
  ...
  geom_pointrange(fatten = 1/2,color = 'red') +
  ...
  geom_hline(yintercept = pop_u2 - pop_u1) + 
...
grouptwo_sd_info <- allocation_data %>% 
  ...
  geom_hline(yintercept = sigma_u2) + 
 labs(x = "Sample Size (Total)",
     y = "Scale Effect Size",
