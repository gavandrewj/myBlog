...
dataset <- filter(dataset,total_phenols != 3.18,flavanoids != 5.08,alcohol != 11.56)
...
The wine samples come from different regions. If some of the samples were brewed in the same batch then this would introduce dependencies between the measurements; this is assumed to not have happened. 
...
group3 <- filter(group3,total_phenols != 2.8,flavanoids != 1.31,alcohol != 14.34 )
group3$cultivar <- 3
...
So we expect that the dependent variables have a linear relationship and a lack thereof would decrease the power of the analysis. From the 3D scatter plot we can notice good separation of the groups; however on the 2D scatterplots presented below four of the nine within group correlations are practically zero. So what kind of linear structure are we really hoping is there when we speak of linearity between the dependent variables?  After what extent can we consider this assumption to be violated. And of course how do we manage this when moving beyond three dependent variables?
...
Overall we consider there to be real differences between the wine samples that come from different regions on all level pairs. Whether these differences are of practical significance is to be determined by someone who can interpret the magnitudes of the paired differences appropriately.  
