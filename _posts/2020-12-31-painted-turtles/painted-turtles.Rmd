...
description: |
  An application of the Hotelling T2 test where the measurements of the carapaces of painted turtles are compared across sex.
...
We can see the separation noted in the earlier beanplot now reflected in the lower left portion of each plot. Still, most of the features are fitted well by a normal distribution curve. The data should be inspected for subgroup but the technique may still generalize well to the question at hand.
...
```
If the outlier identifed in the previous plot is removed we now see a strange pattern emerging where the lower range of points are above the distribution fitted lines while the upper range are below the line. I'm not sure if this has any meaning whatsoever. This will for other purposes, satisfy the normality assumption. 
